...
# شماره کارت را اینجا خودت قرار بده
...
# پیام بعدی مدیر به عنوان دلیل رد ثبت می‌شود.
...
async def normal_message(bot: Robot, message: Message):
    ...
    if chat_id == ADMIN_CHAT_ID:
        ...
        # اگر مدیر در حالت نوشتن دلیل رد رسید است
        if chat_id in admin_reject_waiting:
            ...
            await bot.send_message(
                ...
                "📝 توضیحات مدیریت:\n"
                f"{text}\n\n"
                "لطفاً در صورت نیاز رسید صحیح را ارسال کنید."
            )
    ...
    if order["step"] == "account_id":
        ...
        if not text:
            await message.reply(
                "⚠️ لطفاً آیدی اکانت را به صورت متنی ارسال کنید."
            )
            return
    ...
    if order["step"] == "gmail":
        ...
        if not text:
            await message.reply(
                "⚠️ لطفاً ایمیل را به صورت متنی ارسال کنید."
            )
            return
        ...
        if not re.match(
            ...
            await message.reply(
                ...
                "لطفاً فقط آدرس ایمیل را ارسال کنید."
            )
            return
    ...
    if order["step"] in [
        ...
        await message.reply(
            "ℹ️ این سفارش در حال پردازش یا تکمیل شده است.\n\n"
            "برای مشاهده وضعیت، از گزینه «پیگیری سفارش» استفاده کنید."
        )
...
async def send_payment_info(chat_id):
    ...
    await bot.send_message(
        ...
        f"{CARD_NUMBER}\n\n"
        "━━━━━━━━━━━━━━\n\n"
        "🧾 لطفاً بعد از پرداخت، رسید پرداخت را "
        "به صورت عکس یا متن ارسال کنید."
    )
...
async def notify_admin(tracking_code):
    ...
    # اول متن سفارش را می‌فرستیم
    await bot.send_message(
        ADMIN_CHAT_ID,
        admin_text
    )

    # سپس کی‌پد تأیید/رد را جداگانه می‌فرستیم
    # این روش از نظر سازگاری مطمئن‌تر است.
    await bot.send_message(
        ADMIN_CHAT_ID,
        "👇 عملیات رسید این سفارش:",
        chat_keypad=admin_receipt_keypad(tracking_code),
        chat_keypad_type="New"
    )


# =========================
# callback همه دکمه‌ها
# =========================
    ...
    if button_id.startswith("confirm_"):
        ...
        if package["type"] == "info":
            ...
            await message.reply(
                ...
                "[ ]\n\n"
                "━━━━━━━━━━━━━━\n\n"
                "📧 لطفاً فقط آدرس جی‌میل ، گذرواژه ، کد پشتیبانی بعد اسم و ایدی اکانت را ارسال کنید.\n"
                "⚠️ جیمیل و گذرواژه ات رو همراه شات 10 تایی کد پشتیبانی یکجا ارسال کن و همزمان چسبیده."
            )
...
@bot.on_message(commands=["orders"])
async def orders_command(bot: Robot, message: Message):

    if message.chat_id != ADMIN_CHAT_ID:

        await message.reply(
            "❌ این دستور فقط برای مدیریت است."
        )
        return

    if not orders_by_tracking:
    ...
    text = "📦 سفارش‌های ثبت‌شده:\n\n"

    for tracking_code, order in orders_by_tracking.items():
        ...
        text += (
            f"🎫 {tracking_code}\n"
            f"💎 {package.get('name', 'نامشخص')}\n"
            f"💰 {package.get('price', 'نامشخص')}\n"
            f"📦 وضعیت: {order.get('status', 'نامشخص')}\n\n"
        )
...
# =========================
# ساخت سفارش هنگام اولین
# ثبت tracking
# =========================

def prepare_tracking(order):
...
# =========================
# جایگزین notify_admin برای
# اطمینان از ثبت tracking
# =========================

_original_notify_admin = notify_admin
...
async def notify_admin(tracking_code):
    ...
    await bot.send_message(
        ADMIN_CHAT_ID,
        "👇 برای این سفارش یکی از گزینه‌های زیر را انتخاب کنید:",
