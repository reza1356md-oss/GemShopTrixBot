...
def buy(bot: Robot, message: Message):
    ...
    message.reply_keypad(text, gems_page_1())


@bot.on_callback("gem_110")
def gem_110(bot: Robot, message: Message):
    message.reply("🛒 انتخاب شد: 110 Gem — 275✨\n\nبرای تکمیل خرید، پشتیبانی با شما ارتباط خواهد گرفت.")


@bot.on_callback("gem_231")
def gem_231(bot: Robot, message: Message):
    message.reply("🛒 انتخاب شد: 231 Gem — 518✨\n\nبرای تکمیل خرید، پشتیبانی با شما ارتباط خواهد گرفت.")


@bot.on_callback("gem_583")
def gem_583(bot: Robot, message: Message):
    message.reply("🛒 انتخاب شد: 583 Gem — 1375✨\n\nبرای تکمیل خرید، پشتیبانی با شما ارتباط خواهد گرفت.")


@bot.on_callback("gem_1060")
def gem_1060(bot: Robot, message: Message):
    message.reply("🛒 انتخاب شد: 1060K Gem — 2450✨\n\nبرای تکمیل خرید، پشتیبانی با شما ارتباط خواهد گرفت.")


@bot.on_callback("gem_2180")
def gem_2180(bot: Robot, message: Message):
    message.reply("🛒 انتخاب شد: 2180K Gem — 5,119✨\n\nبرای تکمیل خرید، پشتیبانی با شما ارتباط خواهد گرفت.")


@bot.on_callback("next_page")
def next_page(bot: Robot, message: Message):
    message.reply("⏳ صفحه دوم به‌زودی اضافه می‌شود.")


@bot.on_callback("orders")
def orders(bot: Robot, message: Message):
    message.reply("📦 سیستم پیگیری سفارش به‌زودی فعال می‌شود.")


@bot.on_callback("support")
def support(bot: Robot, message: Message):
    message.reply("🆘 برای پشتیبانی با مدیر فروشگاه در ارتباط باشید.")
